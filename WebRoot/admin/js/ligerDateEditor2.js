...
(function ($)
{
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            var textFlag=true; //鼠标是否在文本框和按钮上jingkew2014-3-30
            var timeFlag=true; //鼠标是否在日历上jingkew2014-3-30
            ...
            var dateeditorHTML = "";
            dateeditorHTML += "<div class='' style='display:none'>";
           /* dateeditorHTML += "    <div class='l-box-dateeditor-header'>";
            ...
            {
				WdatePicker({el:p.inputId,dateFmt:p.format});
                g.inputText.focus();
            });
            //不可用属性时处理
            if (p.disabled)
            ...
            }).click(function(){ //jingkew2014-3-30增加点击事件
            	if(p.readonly) return; //jingkew2014-4-24修改查看页面设置readonly还可以展开日历错误
            	if(p.disabled) return;
            	WdatePicker({el:p.inputId,dateFmt:p.format});
            });
            g.text.hover(function ()
            {
                ...
                textFlag=false; //鼠标在文本和按钮上jingkew2014-3-30
            }, function ()
            {
                g.text.removeClass("l-text-over");
                textFlag=true; //鼠标不在文本和按钮上jingkew2014-3-30
            });
            //LEABEL 支持
            if (p.label)
            {
            ...
            g.set(p);
            //jingkew2014-3-30增加点击事件
            $(document).bind("click.dateeditor", function (e)
            {
            	//满足这三个条件则在其他地方发生单击事件
            	if (textFlag&&timeFlag&&g.dateeditor.is(":visible"))
                {
            		g.toggleDateEditor(true);
                }
            });
        ...
        _getValue: function ()
        {
            return this.inputText.val();//景科文修改日期控件取值方法
        },
        setEnabled: function ()
        {
            var g = this, p = this.options;
