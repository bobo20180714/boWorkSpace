...
$(function(){
	//Tab
	$("#frameTab").ligerTab({ height: "425px" });
	//sql条件form
	formData = $("#sqlForm").ligerForm({
        ...
        validate:true,
        fields: [
			{ display: "组合条件", name: "sqlTest",id:"sqlTest"}
        ]
    });
	//popup
...
function selectDevice(){
	...
	selectDevWin = $.ligerDialog.open({ 
		...
		height : 380, 
        title:'[选择遥测参数]信息',
		url: basePath+'alarm/alarmCfg/selectCondition/selectParam.jsp',
		buttons :[
...
function saveSql(){
	...
	for(var i = 0;i < sql.length; i++){
		var right="",left="";
//		if(sql[i].indexOf("AND")>-1||sql[i].indexOf("OR")>-1||sql[i]=="("||sql[i]==")") continue;
...
//		sql[i] = left + " '" + deviceNo + "'.'TM'.'" + paramCode + "' " + condition + " " + deviceValue + right;
		sql[i] = left + paramCode + " " + condition + " " + deviceValue + right;
	}
	sqlWhere = sql.join(" ");
	//调用父窗口方法，关闭窗口
	condWin.closeWin_2(sqlWhere);
}
