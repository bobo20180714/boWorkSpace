		...
		return {
			storeToXml : function(grid, title, sql) {
				...
				if(sql!= null && sql!=undefined && sql !=""){
					var sqlXML = "";
					//temp = '<ss:Worksheet ss:Name="sql">';
					sqlXML += '<ss:Row>';
					sqlXML += '<ss:Cell ss:MergeAcross="'+ (columns.length - 1)	+ '"><ss:Data ss:Type="String">';
					sqlXML += sql;
					sqlXML += '</ss:Data></ss:Cell>';
					sqlXML += '</ss:Row>';
					//temp += '</ss:Worksheet>';
				}
				var headerXml = '<ss:Cell ss:StyleID="headercell" ss:MergeAcross="'+ (columns.length - 1)	+ '">'
						+ '<ss:Data ss:Type="String">'	+ title+ '</ss:Data>'
						+ '<ss:NamedCell ss:Name="Print_Titles" />'
						+ '</ss:Cell>';
				temp += '<ss:Table ss:DefaultColumnWidth="200">' + '<ss:Column ss:AutoFitWidth="1"/>'
						+ sqlXML 
