...
Ext.onReady(function () {
    ...
    ctx.StrokeText = function (str, x, y, color) {
        ...
        x = x * 1 + left_ - 80;
        y = y * 1 + top_;
        el.style.cssText = 'position:absolute;overflow:hidden;text-align:center;font-size:12px;left:' + x + 'px;top:' + y + 'px;width:120px;height:14px;color:' + color+';white-space:nowrap;';
        el.innerHTML = str;
    ...
    ctx.StrokeLegend=function(str,x,y){
        ...
        x = x * 1 + left_;
        y = y * 1 + top_;
        el.style.cssText = 'position:absolute;overflow:hidden;padding:5px;font-family:"楷体","楷体_GB2312";font-size:14px;left:' + (x+1) + 'px;top:' + y + 'px;width:155px;height:'+(h/10-2)+'px;color:#000;';
        el.innerHTML = str;
        owner.appendChild(el);
    };
    ...
    var url = '../rest/drawdata/getdrawdata';
    for (i = 0; i < info.Lines.length; i++) {
        ...
        if(info.Lines[i].ParamType==1){
        	url = '../rest/drawdata/customdata';
        }else{
        	url = '../rest/drawdata/getdrawdata';
        }
        Ajax.send(url,{
            ...
            start:Date.toStr(info.Lines[i].Begin)
        }, function (json, opts) {
