(function($){
	var bigColorPicker = new function(){
		...
		this.init = function(){
			$("body").append('<div id="bigpicker" class="bigpicker"><ul class="bigpicker-bgview-text" ><li><div id="bigBgshowDiv"></div></li><li style="padding:2px"><input id="bigHexColorText" size="7" maxlength="7" value="#000000" /></li><li><label id="bigTransparent" style="display:none"><input type="checkbox" style="width:20px">透明</label></li></ul><div id="bigSections" class="bigpicker-sections-color"></div><div id="bigLayout" class="biglayout" ></div></div>');			
		 ...
		 */
		this.showPicker = function(callback,engine,sideLength_,clickTransparent){
			...
			if(engine == "L" ){
				...
				//预加载图片
			     ...
			     bgmage.src = "colorpicker/images/big_bgcolor.jpg";
			     //初始化为默认样式
			...
			$(this).bind("click",function(event){
				...
				if(clickTransparent){
					$('#bigTransparent').show().find('input').prop('checked',$(this).data('color')=='transparent')[0].onclick=function(){
						clickTransparent(this.checked);
					};
				}
				else{
					$('#bigTransparent').hide().find('input')[0].onclick=null;
				}
			});			
		};
