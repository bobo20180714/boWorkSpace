...
function selectTabItem(e){
	if(e == "tabitem2"){
		if(!isInitRunParam){
			...
			var paramArr = funcDataInfo.fieldList;
			if(funcDataInfo.isUserDefined == 1){
				runParamForm.setData(funcDataInfo);
        ...
        fields: [
           /*  { display: "模块代号", name: "fctCode",id:"fctCode",
			...
			{ display: "功能名称", name: "computeName",id:"computeName",
				type: "text", newline: false,validate:{required:true,maxlength:200}
			},
...
function initRunParamForm(){
	runParamForm = $("#runParamForm").ligerForm({
		...
		validate:true,
		fields: [
		         { display: "超时时间(分钟)", name: "overTime",id:"overTime",
		        	 type: "select", newline: true,validate:{required:true},
		        	 data:overTimeArr
		         },
		         { display: "最大失败次数", name: "computeCount",id:"computeCount",
...
function submitForms() {
	...
	if(toValidForm(data)){
		if(data.isUserDefined == 1 && runParamForm == null){
		...
		data.pkId = computFuncId;
		$.post(basePath+"rest/ComputeFunc/update",data,function(rsData){
			var jsobj = eval('('+rsData+')');
			if(jsobj.success=='true'){
				parent.Alert.tip("修改计算模块成功!");
				parent.f_closeAndLoad();
			}else{
				parent.Alert.tip("修改计算模块失败!");
			}
		});
	}else{
		return false;
...
function getFctData(){
	$.ajax({
		...
		data:{
			computeId:computFuncId
		},
		async:false,
		success:function(rsData){
			funcDataInfo = eval('('+rsData+')');
			computeInfoForm.setData(funcDataInfo);
			selectIsGetData({
				value:funcDataInfo.isUserDefined
			});
		}
	});
}
