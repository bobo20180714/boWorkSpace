...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if(p.onBrowser){
            	g.selectBox.table.after('<div style="cursor:pointer;padding:2px;">更多···</div>');
            	g.selectBox.table.next().click(p.onBrowser).hover(function(){
        ...
        _setSelectBoxHeight: function (height)
        {
            ...
            } else if (!p.tree)
            {
                ...
                if (p.selectBoxHeight && g.selectBoxInner)
                {
                    if (itemsleng < 8)
                    {
        ...
        _setValue: function (value, text)
        {
            ...
            {
                g._changeValue(value, text, isTriggerEvent, p.noTriggerSelectedEvent);
                    ...
                    if (g.trigger('beforeSetData', [data]) == false)
{
        ...
        _dataInit: function ()
        {
            ...
            //根据值来初始化
            if (p.initValue != null && p.initValue != "")
            {
                value = p.initValue;
                if (p.tree)
                {
            ...
            }else if(g.data&&g.data.length>0&&p.selectedIndex!==null){
            	var val=g.data[0][p.valueField];
            	g._changeValue(val, g.data[0][p.textField]);
            	$("tr[value='" + val + "'] td", g.selectBox).addClass("l-selected");
            	g.selected=g.data[0];
            }
        ...
        _changeValue: function (newValue, newText,isSelectEvent,noTriggerSelectedEvent)
        {
            var g = this, p = this.options;
            ...
            if (!noTriggerSelectedEvent && isSelectEvent)
            {
