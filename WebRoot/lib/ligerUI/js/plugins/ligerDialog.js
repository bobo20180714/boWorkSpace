...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if (p.buttons)
            {
                $(p.buttons).each(function (i, item)
                {
					...
					if(item.type){
						if(item.type=='save'){
							btn = $('<div class="l-dialog-save-btn"></div>');
						}else if(item.type=='close'){
							btn = $('<div class="l-dialog-close-btn"></div>');
						}
					}
        ...
        {
        	if(this.options.type=='tip' && content)
        		content = '<center style="width: 140px; margin-left:40px; margin-top: 15px; margin-bottom: 10px;line-height: 30px;">'+content+'</center>';
        ...
        show: function ()
        {
            ...
            if (p.fixedType)
            {
                if (p.showType)
                {
                	g.dialog.css({ 'margin-left':'-120px' });
                    g.dialog.css({ top: 0 }).addClass("l-dialog-fixed");
                    g.dialog.show().animate({ top: 150 });
                }
                else
                {
