...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getChecked: function ()
        {
            ...
            var nodes = [];
            $(".l-checkbox-checked", g.tree).parent().parent().parent("li").each(function ()
            {
        ...
        update: function (domnode, newnodedata)
        {
            ...
            for (var attr in newnodedata)
            {
                ...
                {
                    //mengxiangchao  update 20170607
                    $("> .l-body-inner > span", domnode.firstChild).text(newnodedata[attr]);
                }
            }
        },
        //增加节点集合
        ...
        append: function (parentNode, newdata, nearNode, isAfter)
        {
            ...
            {
                var gridhtmlarr = [];
                gridhtmlarr = gridhtmlarr.concat(g._getTreeHTMLByData(newdata, 1, [], true));
        ...
        selectNode: function (selectNodeParm,isTriggerEvent)
        {
            ...
            else if (typeof (selectNodeParm) == "object")
            {
                ...
                {
                    treeitembody = $("li[treedataindex=" + treedataindex + "] >div:eq(0)", g.tree);
                }
                if (p.checkbox)
                {
                ...
                else
                {
                    ...
                    $("div.l-body-bg-non", g.tree).removeClass("l-body-bg-non");
                    if(!treeitembody.parent().prevAll("li").length && treeitembody.parent().parent().hasClass("l-children")){
                    	treeitembody.parent().parent().prevAll("div:eq(0)").addClass("l-body-bg-non");
                    }else{
                    	treeitembody.parent().prevAll("li:eq(0)").children("div:eq(0)").addClass("l-body-bg-non");
                    }
                }
                if (isTriggerEvent != false)
                {
        ...
        {
            var g = this, p = this.options;
            var treedataindex = parseInt($(treeNode).parent('li').attr("treedataindex"));
            var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
		...
		_getTopNodeClassName: function (isOpen)
        {
            var g = this, p = this.options;
            var nodeclassname = 'l-tree-icon-' + p.topIcon;
            if (isOpen) nodeclassname += '-open';
            return nodeclassname;
        },
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                //treehtmlarr.push('<div class="l-span-line l-span-line-hide"></div>');
                treehtmlarr.push('<li ');
                ...
                treehtmlarr.push('<div class="l-body l-body-bg');
                if (p.selectable && p.selectable(o) == false)
                {
                    treehtmlarr.push(' l-unselectable');
                }
                treehtmlarr.push('">');
				treehtmlarr.push('<div class="l-body-inner">');
                ...
                if (g.hasChildren(o))
                {
					...
					if(p.topIcon && (!o[p.parentIDFieldName] || o[p.parentIDFieldName] == -1)){
						//顶级节点图标
						treehtmlarr.push('<div class="l-box l-tree-icon ');
						treehtmlarr.push(g._getTopNodeClassName(isExpandCurrent ? true : false) + " ");
                ...
                else
                {
                	...
                	else if (isLastCurrent) treehtmlarr.push('<div class="l-box l-note-last"></div>');
                ...
                if (p.render)
                {
                    treehtmlarr.push('<span  title="'+o[p.textFieldName]+'" style="width:150px;">' + p.render(o, o[p.textFieldName]) + '</span>');
                } else
                {
                    treehtmlarr.push('<span  title="'+o[p.textFieldName]+'" style="width:150px;">' + o[p.textFieldName] + '</span>');
                }
				treehtmlarr.push('</div>');
                treehtmlarr.push('</div>');
        ...
        _setTreeEven: function ()
        {
            ...
            if (g.hasBind('contextmenu'))
            {
                g.tree.bind("contextmenu", function (e)
                {
                    ...
                    else if ($(obj).hasClass("l-body-inner"))
            ...
            g.tree.click(function (e)
            {
                ...
                if (!$(obj).hasClass("l-checkbox") && !clickOnTreeItemBtn)
                {
                    ...
                    {
                        if ($(">div:eq(0)", treeitem).hasClass("l-selected") && p.needCancel)
                        {
                            if (g.trigger('beforeCancelSelect', [{ data: treenodedata, target: treeitem[0] }]) == false)
                                return false;

                            $(">div:eq(0)", treeitem).removeClass("l-selected");
                            $(".l-body", g.tree).removeClass("l-body-bg-non");
                            g.trigger('cancelSelect', [{ data: treenodedata, target: treeitem[0] }]);
                        }
                        else
                        {
                            ...
                            $(".l-body", g.tree).removeClass("l-body-bg-non");
                            if(!treeitem.prevAll("li").length && treeitem.parent().hasClass("l-children")){
                            	treeitem.parent().prevAll("div:eq(0)").addClass("l-body-bg-non");
                            }else{
                            	treeitem.prevAll("li:eq(0)").children("div:eq(0)").addClass("l-body-bg-non");
                            }
                            
                            $(">div:eq(0)", treeitem).addClass("l-selected");
                ...
                else if (treeitembtn.hasClass("l-expandable-open") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn))
                {
					...
					$("> div ." + g._getTopNodeClassName(true), treeitem)
                            .removeClass(g._getTopNodeClassName(true))
                            .addClass(g._getTopNodeClassName());
                ...
                else if (treeitembtn.hasClass("l-expandable-close") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn))
                {
                    ...
                    $("> div ." + g._getParentNodeClassName(), treeitem)
                            ...
                            .addClass(g._getParentNodeClassName(true));
					$("> div ." + g._getTopNodeClassName(), treeitem)
                            .removeClass(g._getTopNodeClassName())
                            .addClass(g._getTopNodeClassName(true));
                }
