...
var whereConditionObj = null;

$(function(){
	
	initConditionForm();
	
	whereConditionObj = $("#whereCondition").ligerTextBox({
		width:445
	});
});
...
function getConditionInfo(){
	...
	if(!formData.sat_name){
		parent.Alert.tip("请选择所属卫星！");
		return null;
	}
	relateId = formData.relateName;
	if(!relateId){
		parent.Alert.tip("请选择数据类型！");
		return null;
	}
	if(!formData.startTime){
		parent.Alert.tip("请选择开始时间！");
		return null;
	}
	if(!formData.endTime){
		parent.Alert.tip("请选择结束时间！");
		return null;
	}
	var relateType = getCodeFromRelateData(relateId);
...
function addTr(){
	$("#paramTable").append('<tr  id="tr_'+index+'"  style="height:30px;">'+
			'	<td width="10px"><input id="label_'+index+'"'+
			'		name="label_'+index+'" value="'+index+'"  type="hidden"/>'+index+'</td>'+
...
function groupCondition(){
	...
	for (var i = 0; i < trNodeList.length; i++) {
		...
		var nowRowNum = trId.split("_")[1];
		var trNum = $("#label_"+nowRowNum).val();
		var lianjie = $("#lianjie_"+nowRowNum).val();
		if(i+1 < trNodeList.length){
			var trId2 = trNodeList[i+1].id;
			var nowRowNum2 = trId2.split("_")[1];
			var trNum2 = $("#label_"+nowRowNum2).val();
			if(i == 0){
				where = trNum + " " + lianjie + " " + trNum2 + " ";
			}else{
				where = where + " " + lianjie + " " + trNum2 + " ";
			}
		}
	}
	whereConditionObj.setValue(where);
}
	...
	conditionForm = $("#form1").ligerForm({
        ...
        fields: [
             { 
         		...
         		onButtonClick:openSatWin,validate:{required:true}
             },
			...
			{ display: "开始时间", name: "startTime",id:"startTime",showTime:true,
				type: "date", newline: true,validate:{required:true}
			},
			{ display: "结束时间", name: "endTime",id:"endTime",showTime:true,
				type: "date", newline: false,validate:{required:true}
			}
        ]
	});
}
	...
	var satSatWin = parent.$.ligerDialog.open({
		...
		buttons :[
		          { text: '确定', type:'save', width: 80,onclick:function(item, dialog){
						...
						//根据卫星ID获取数据类型
						getRelateData(data.satId);
//						liger.get('relateName').setData(getRelateData(data.satId));
						debugger;
						liger.get('relateName').setParm("satId",data.satId);
						liger.get('relateName').reload();
						satSatWin.close();
		          }},
