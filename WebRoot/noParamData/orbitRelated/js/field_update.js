	...
	formData = $("#form1").ligerForm({
        ...
        fields: [
			...
			/*{ display: "是否为摘要信息", name: "is_display_flag",id:"is_display_flag",
    			 ...
    			 initValue:0,newline: true,validate:{required:true}},*/
...
function setFormData(){
	$.ajax({
		url:basePath+"rest/orbitrelated/viewField",
		data:{
			field_id:field_id
		},
		async:false,
...
function submitForm(){
	...
	if(toValidForm()){
		...
		$.ajax({
			url:basePath+"rest/orbitrelated/updateField",
			data:data,
			async:false,
			success:function(data){
				var rsData = eval('('+data+')');
