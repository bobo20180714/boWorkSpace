...
function setGetDataInfo(){
	setParamInfo("paramTable_getData",orderData.getDataParamList);
	//设置获取数据函数显示值
	liger.get("get_data_func_name").setText(orderData.getDataFunName);
	liger.get("get_data_func_name").setValue(orderData.getDataId);
	//设置组合参数数据
	$("#getData_paramxml").ligerGetTextBoxManager().setValue(orderData.getDataParam);
	
}


function setComputInfo(){
	setParamInfo("paramTable_comput",orderData.computParamList);
	//设置数据计算函数显示值
	liger.get("comput_func_name").setText(orderData.computFunName);
	liger.get("comput_func_name").setValue(orderData.computId);
	//设置组合参数数据
	$("#comput_paramxml").ligerGetTextBoxManager().setValue(orderData.computParam);
}


function setResultInfo(){
	setParamInfo("paramTable_result",orderData.resultParamList);
	//设置数据处理函数显示值
	liger.get("result_func_name").setText(orderData.resultFunName);
	liger.get("result_func_name").setValue(orderData.resultId);
	//设置组合参数数据
	$("#result_paramxml").ligerGetTextBoxManager().setValue(orderData.resultParam);
}

/**
 * 参数区域赋值
 */
function setParamInfo(paramTableId,paramList){
	var tableBody = $("#"+paramTableId)[0].childNodes;
	var trNodeList = tableBody[1].childNodes;
	var num = null;
	for (var i = 0; i < trNodeList.length; i++) {
		var trId = trNodeList[0].id;
		var trIdArr = trId.split("_");
		num = trIdArr[1];
	}
	var paramLength = paramList.length;
	var addCount = paramLength - trNodeList.length;
	for (var i = 0; i < addCount; i++) {
		if(i == 0){
			addParam(paramTableId, num);
		}else{
			addParam(paramTableId, index);
		}
	}
	tableBody = $("#"+paramTableId)[0].childNodes;
	trNodeList = tableBody[1].childNodes;
	for (var i = 0; i < trNodeList.length; i++) {
		var param = paramList[i];
		var trId = trNodeList[i].id;
		var trIdArr = trId.split("_");
		num = trIdArr[1];
		$("#param_type_"+num).ligerGetComboBoxManager().setValue(param.paramType);
		if(param.paramType == "0"){
			$("#param_value_"+num).ligerGetPopupEditManager().setText(param.paramValue);
			$("#hide_param_value_"+num).val(param.tmValue);
		}else if(param.paramType == "1"){
			$("#param_value_"+num).ligerGetTextBoxManager().setValue(param.paramValue);
		}else if(param.paramType == "2"){
			$("#1_param_value_"+num).ligerGetDateEditorManager().setValue(param.timeStart);
			$("#2_param_value_"+num).ligerGetDateEditorManager().setValue(param.timeEnd);
		}
	}
}
...
function addTr(paramTableId){
	$("#"+paramTableId).append('<tr  id="tr_'+index+'"  style="height:30px;">'+
			...
			'	<td id="td_value_'+index+'" width="260px"><input type="text" id="param_value_'+index+'"'+
 ...
 * @param nowRowNum
 */
function getParamXml_2(nowRowNum,isDelete){
	var paramXmlStr = "";
	...
	for (var i = 0; i < childNodesArr.length; i++) {
		...
		if(paramType == 0){
			...
			if(hideValue){
				...
				xmlStr = xmlStr + '<TM><satMid>'+satData.satMid+'</satMid><tmList>';
	...
	orderForm = $("#orderForm").ligerForm({
        ...
        fields: [
             { display: "订单名称", name: "orderName_1",id:"orderName_1",
            	 type: "text", newline: true,validate:{required:false,maxlength:200}
             },
			{ display: "是否需要数据获取模块", name: "isGetData_1",id:"isGetData_1",
				type: "radiolist", newline: true,onSelect:selectIsGetData,
				data:[{id:"0",text:"否"},{id:"1",text:"是"}]
			},
			{ display: "是否需要结果处理模块", name: "isResult_1",id:"isResult_1",
				type: "radiolist", newline: true,onSelect:selectIsResult,
				data:[{id:"0",text:"否"},{id:"1",text:"是"}]
			},
	...
	computerForm = $("#computerForm").ligerForm({
		...
		fields: [
		         { display: "计算模块", name: "comput_func_name",id:"comput_func_name",
		        	 type: "popup", newline: false,validate:{required:false},
...
function submitForms(){
	...
	if (vboolean) {
		...
		selectTabItem("tabitem2");
		selectTabItem("tabitem3");
		selectTabItem("tabitem4");
		
		var orderInfo = orderForm.getData();
		orderInfo.pkId = orderData.pkId;
		if(orderInfo.isGetData_1 == "1"){
			if(getDataForm == null){
				parent.Alert.tip("请输入数据获取模块信息！");
				return false;
			}
			//获取数据方法信息
	...
	$("#hideForm").ajaxSubmit({
		...
		success:function(data){
			...
			if(jsobj.success == "true"){
				parent.Alert.tip("订单编辑成功！");
         		//刷新表格
         		parent.closeUpdateWinAndLoadGrid();
			}else{
				parent.Alert.tip("订单编辑失败！");
			}
		}
	});
}
...
function getOrderData(){
	$.ajax({
		url:basePath+'rest/orderManager/getOrderDetail',
		data:{
			orderId:orderId,
		},
		async:false,
		success:function(rsStr){
			...
			//显示订单基本信息
			orderForm.setData({
				orderName_1:orderData.orderName,
				isGetData_1:orderData.isGetData,
				isResult_1:orderData.isResult,
				orderContent_1:orderData.orderContent,
			});
			//设置卫星数据
			liger.get('sat_name').setText(orderData.satName);
			liger.get('sat_name').setValue(orderData.satMid);
			satData = {
				satId:orderData.satId,
				satName:orderData.satName,
				satMid:orderData.satMid
			};
			//触发是否需要获取数据函数事件
			selectIsGetData({
				value:orderData.isGetData
			});
			//触发是否需要数据处理函数事件
			selectIsResult({
				value:orderData.isResult
			});
		}
	});
}