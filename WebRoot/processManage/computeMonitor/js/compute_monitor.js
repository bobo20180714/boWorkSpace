...
//	var nodes = satGroupTree.getChecked();
	...
	$("#hideForm").ajaxSubmit({
		url:basePath+'rest/monitor/queryChangeCompute',
	...
	if (item.id) {
		...
		case "add":
			addWin = $.ligerDialog.open({
				...
				title : "注册服务器",
				isResize : false,
				url : basePath+"processManage/computeMonitor/compute_add.jsp",
				buttons:[
		...
		case "delete":
			...
			if(processState == 1){
				Alert.tip('工作中的服务器不能删除！');
				return;
			}
			$.ligerDialog.confirm('确定要删除该服务器吗？',function (yes){
				if(yes){
					$.ajax({
		...
		case "view":
			...
			var viewWin = $.ligerDialog.open({
				...
				title : "查看进程",
				isResize : false,
				url : basePath+"processManage/computeMonitor/compute_view.jsp?processCode="+processCode,
				buttons:[
