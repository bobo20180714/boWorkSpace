...
var _tagid = 1;
function create(c,id,name,aw,type,lockState){
	var id = id!=undefined?id:'tag'+myId.getVal(),
		el = document.createElement("div"),
		txt = document.createElement("div"),
		ta = document.createElement("textarea"),
		readOnly = true,
		lock = lockState == null?false:lockState,
	    ...
	    setLock = function(){
	    	if(popMenu){
	    		popMenu.items.items[0].hide();
	    		popMenu.down('#unlock').show();  
            	lock = true;
    ...
    txt.innerHTML=name == null?'新标注'+_tagid++:name;
    el.appendChild(txt);
    ta.rows='4';
